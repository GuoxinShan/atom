...
import { IRRELEVANT_REASON, MUTED_SOURCE_REASON } from "./irrelevant.js";
...
export function reopenCandidate(store: EventStore, candidateId: string, note?: string): CandidateView {
  ...
  const muted = found.disposition === MUTED_SOURCE_REASON || found.reject_reason === MUTED_SOURCE_REASON;
  if (
    found.disposition !== ALREADY_DONE_REASON &&
    !isAlreadyDoneReason(found.reject_reason) &&
    !irrelevant &&
    !muted
  ) {
    throw new Error(`Only already_done, irrelevant, or muted_source items can be reopened: ${candidateId}`);
  }
  store.append({
