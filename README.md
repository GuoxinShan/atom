...
Same daemon, same SQLite. Home is **需要你拍板** (Approve/Reject). Cards fold into collapsible **theme/project groups**. After extract, Laya labels new candidates with human `theme`/`project` (Chinese titles like 「AI推进」 when it is up). If Laya is down, cards stay untagged and cluster by workspace + title stem / `cluster_key` (same as #24). Approve/Reject is still per card. Hard-refresh `http://127.0.0.1:8787` after pull. **系统已处理** is a read-only 24h gate-digest. **我的偏好** shows `data/preference-memory.json`. **高级** hides the Atoms log placeholder. Lead NL configures sources.
