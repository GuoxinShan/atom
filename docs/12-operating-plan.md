...
- **Done gate** (after noise/merge, before theme tag) matches suggested cards against `data/progress-snapshot.json` (atom / yzj / ai-advance merged PRs, closed issues, recent main\|master commits) plus Desk accept/reject/merge history plus high-confidence 云之家 completion talk (`discourse` on that snapshot: 已完成 / 搞定 / 上线了, from messages already ingested — not a second pull). Repo title/stem hits need distinctive stems plus a title/body similarity floor or workspace affinity (failure words such as 失败 are generic; 云之家 is one phrase, not 云之 + 之家); atom meta PRs (Desk / Done-gate / chore) are not evidence that yzj / ai-advance product work is done. Chat hits need the completion subject to cover the card title and must not cross themes. Hit → `already_done` (off Needs-you). Repo/history reason 「已在仓库/历史进度关闭」; 云之家 reason 「云之家进度关闭」. Uncertain, vague chat, theme mismatch, or repo-scan fail → stay suggested
