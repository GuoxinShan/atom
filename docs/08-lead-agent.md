...
3. **Extract → theme/project tags** — after a candidate is kept as suggested (not noise, not auto-merged), `POST /v1/predict` with typed `theme` / `project` choice heads. Persist human titles on `candidate_proposed` (`theme` / `project` and `tags.*`, plus `laya_tags` audit) so Desk `GET /api/candidates` → `groups` prefers them over the workspace+title heuristic. Prefer Chinese titles (「AI推进」, not Schedule Mcp / ai-advance). Timeout, 5xx, Laya down, or parse fail → **fail-open**: keep the candidate as-is (Grok tags if any; otherwise untagged heuristic grouping). Never blocks extract. Display grouping only — not a merge.
