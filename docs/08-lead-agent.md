...
6. **Lead handoff → model route** — before coding handoff, `POST /v1/route-model` with the task summary. `ornith` = heavier, `bonsai` = lighter; recorded on `handoff_exported` as `laya_model_route`. No intensity-split coding providers yet, so handoff is unchanged.
