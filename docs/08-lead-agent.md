...
2. **Extract → duplicate merge gate** — after the noise filter, `POST /v1/predict` again with the new candidate plus a short list of open Needs-you / suggested items **ranked by local title/body/ref near-duplicate score** (cap still 8). Auto-merge when `same_request` ≥ **0.90**, or when the pair is a local near-duplicate and `same_request` ≥ **0.72** (paraphrases like 速记迁灵基鉴权). High-confidence `merge` then folds the duplicate into that existing item (`decision_merged` on the loser; survivor stays suggested; survivor inherits a canonical theme if it was 「其他」). Far-apart titles (e.g. 速记 vs 日程 MCP) force `new` even if `same_request` is high. Ambiguous, timeout, or Laya down → fail-open and create a new suggested ticket (never drop the card). Desk remains the accept/reject gate.
3. **Extract → theme/project tags** — after a candidate is kept as suggested (not noise, not auto-merged), `POST /v1/predict` with typed `theme` / `project` choice heads whose criteria are the **closed Chinese vocabulary** in [`data/theme-vocabulary.json`](../data/theme-vocabulary.json) (product-editable). Persist allowlist titles on `candidate_proposed` (`theme` / `project` and `tags.*`, plus `laya_tags` audit) so Desk `GET /api/candidates` → `groups` prefers them over the workspace+title heuristic. Laya must pick from that list (「AI推进」「日程/会议」「发布与发布流程」…); English kebab slugs (`release-process`, `product-bug`, Schedule Mcp) map onto the nearest allowlist entry when the mapping is unique. Empty/garbage/「其他」 answers **divert** from title/body onto that same closed list (first unique/earliest hit; 速记迁入灵基 → 「速记」, not a new label). Timeout, 5xx, or Laya down → **fail-open**: keep the candidate as-is (Grok tags if any; otherwise untagged). Display grouping still diverts locally so 「其他」 shrinks without dropping cards. Never blocks extract. Display grouping only — not a merge.
...
5. **Needs-you theme/project tag backfill** — one-shot `pnpm atom tag-backfill` (dry-run default; `--apply` writes). Re-runs the same Laya tag gate across **existing** open suggested items that are missing an allowlist theme (untagged heuristic leftovers, 「其他」, or pre-allowlist kebab/free-form titles). Persist overlay `candidate_tagged` with closed Chinese titles from [`data/theme-vocabulary.json`](../data/theme-vocabulary.json); never kebab slugs. Known aliases (`release-process` → 「发布与发布流程」, `日历` → 「日程/会议」) remap locally without Laya. Title/body hits on the same closed list **divert 「其他」** locally (no new vocabulary entries). Timeout / 5xx / Laya down **fail-open per card** (leave unchanged) except local allowlist/divert remaps. Skip accepted/rejected/merged. Idempotent. Desk IA and Docker auth are unchanged.
